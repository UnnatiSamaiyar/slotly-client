...
export default function BookingForm({ userSub }: { userSub: string }) {
    ...
    async function loadSlots() {
      try {
        ...
        const res = await fetch(
          `http://api.slotly.io/bookings/availability?user_sub=${encodeURIComponent(
            userSub
  ...
  const meetingLink = useMemo(() => {
    return `http://slotly.io/dashboard/book?user_sub=${encodeURIComponent(
      userSub
    )}`;
  }, [userSub]);
  ...
  const createBooking = async () => {
    ...
    try {
      const res = await fetch(
        `http://api.slotly.io/bookings/create?user_sub=${encodeURIComponent(
          userSub
        )}`,
        {
          method: "POST",
