...
export default function BookingForm({
  ...
  useEffect(() => {
    ...
    if (!date || !userSub || isActive === false) {
      setSlots([]);
      setSlotsError(null);
      return;
    }
  ...
  const createBooking = async () => {
    ...
    // Show modal immediately (as you wanted)
  ...
  return (
    <div className="relative flex flex-col lg:flex-row">
      ...
      {/* LEFT PREVIEW (desktop sidebar + mobile bottom drawer) */}
      <aside
        className={`
          bg-white
          ...
          <Section title="Date & time">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
              ...
              <Select
                ...
                onChange={(v: string) => setDuration(parseInt(v.split(" ")[0], 10))}
              />
            </div>
              ...
              {time && (
                <div className="mt-3 flex items-center justify-between bg-indigo-50 border border-indigo-200 rounded-lg px-4 py-3">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-indigo-700">{time}</p>
                  </div>
