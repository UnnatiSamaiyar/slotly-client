...
//   const mode = useMemo(() => {
//     return stateObj?.mode === "calendar" ? "calendar" : "login";
//   }, [stateObj]);
...
//     async function run() {
//       try {
//         setMessage(mode === "calendar" ? "Connecting Google Calendar…" : "Completing login…");
//         if (!code) {
//           setStatus("error");
...
//         } else {
//           // Calendar connect must NOT replace the logged-in Slotly user.
//           window.dispatchEvent(new Event("slotly-calendar-changed"));
//         }
//         if (cancelled) return;
//         onAuthReady();
//       } catch (e: any) {
...
//   }, [code, stateStr, returnTo, stateObj, mode]);
...
function GoogleCallbackInner() {
  ...
  const returnTo = useMemo(() => {
    ...
    if (typeof rt === "string" && rt.startsWith("/") && !rt.includes("://")) {
      return rt;
    }
    return "/dashboard";
  }, [stateObj]);
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        const API_BASE = pickApiBase();
        const mode = stateObj?.mode || "login";
        ...
        const res = await fetch(`${API_BASE}/auth/google`, {
          ...
          body: JSON.stringify({
            ...
            mode,
            returnTo,
            user_sub: stateObj?.user_sub || storedSession?.sub || null,
            redirect_uri:
        ...
        if (!res.ok) {
          const backendMessage = await readBackendError(res);

          const friendlyMessage = backendMessage
            .toLowerCase()
            .includes("user not found")
            ? "No Slotly account was found for this Google account. Create an account first, or ask backend to create/link Google users automatically."
            : backendMessage;
        ...
        if (mode === "calendar") {
          ...
          if (existingSession) {
            const updatedSession = {
              ...
              calendar_email:
                payload?.calendar_email ||
                payload?.google_email ||
                existingSession?.calendar_email ||
                "",
            };

            localStorage.setItem("slotly_user", JSON.stringify(updatedSession));
            sessionStorage.setItem("slotly_user", JSON.stringify(updatedSession));
          }
        ...
        if (!sub) {
          setStatus("error");
          setMessage("Login succeeded but user id was not returned by server.");
          return;
        }
