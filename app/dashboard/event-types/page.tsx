...
//   return (
//     <div className="h-screen flex bg-white overflow-hidden">
//       {/* <Sidebar open={sidebarOpen} onToggle={() => setSidebarOpen((s: boolean) => !s)} user={user} /> */}
//       <Sidebar
//         open={sidebarOpen}
...
export default function DashboardEventTypes() {
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!ok) {
      toast({
        title: "Cancelled",
        description: "Event type was not deleted.",
        variant: "default",
      });
      return;
    }
  ...
  return (
    <div className="h-screen flex bg-white overflow-hidden">
      ...
      {/* ✅ IMPORTANT: On mobile, sidebar toggle button is fixed at top-left,
          so we add left padding to content/topbar to keep alignment consistent. */}
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <div className="shrink-0 pl-14 md:pl-0">
          <Topbar user={user} searchQuery="" onSearchQueryChange={() => { }} />
        </div>

        <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 pl-14 md:pl-0">
          {/* Header */}
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              <h1 className="text-2xl font-semibold text-gray-900">{pageTitle}</h1>
            ...
            {tab === "event_types" ? (
              ...
              <>
                ...
                <div className="mt-4">
                  ...
                  {/* (Meetings list UI unchanged from your current version) */}
                </div>

                {/* Details Modal (unchanged from your current version) */}
              </>
            )}
          </div>
...
function EventTypeEditModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await updateEventType(Number(item.id), payload);
