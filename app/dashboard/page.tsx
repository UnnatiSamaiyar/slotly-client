...
// // // // import { useCalendarEvents } from "./hooks/useCalendarEvents";
// // // // import { useUserProfile } from "./hooks/useUserProfile";
// // // // import { isSameISODate, safeDate, toISODateLocal } from "./components/Calendar/CalendarHelpers";
...
// // // //     return events.filter((e: any) => isSameISODate(e.start, selectedDate));
// // // //   }, [events, searchQuery, selectedDate]);
...
// // //   const { data: user } = useUserProfile(userSub);
// // //   const { events, loading, error, refresh } = useCalendarEvents(userSub, "all");

// // //   // Selected date ke liye meetings
// // //   const filteredEvents = useMemo(() => {
// // //     if (!events) return [];
// // //     return events.filter((e: any) => isSameISODate(e.start, selectedDate));
// // //   }, [events, selectedDate]);

// // //   // Calendar dots ke liye ALL events by date
...
// //   // clicked/selected date ki meetings
// //   const selectedDayEvents = useMemo(() => {
// //     if (!events) return [];
// //     return events
// //       .filter((e: any) => isSameISODate(e.start, selectedDate))
// //       .sort((a: any, b: any) => {
...
// //   // calendar dots ke liye all events
// //   const eventsByDate = useMemo(() => {
// //     const map: Record<string, any[]> = {};
...
//     return () => {
//       window.removeEventListener("slotly-dashboard-search", handler as EventListener);
//     };
//   }, []);
...
//         if (!d) return false;
//         return toISODateLocal(d) === selectedDate;
//       })
//       .sort((a: any, b: any) => {
//         const ta = a?.start ? new Date(a.start).getTime() : 0;
//         const tb = b?.start ? new Date(b.start).getTime() : 0;
...
//         e?.organizer || e?.organizer_name || ""
//       ).toLowerCase();
//       const organizerEmail = String(
//         e?.organizer_email || e?.email || ""
//       ).toLowerCase();
...
//         <UpcomingEvents
//           events={searchQuery.trim() ? selectedDayEvents : (events || [])}
//           selectedDate={selectedDate}
//         />
//       </aside>
//     </div>
...
export default function DashboardPage() {
  ...
  const selectedDayEvents = useMemo(() => {
    const base = [...(eventsByDate[selectedDate] || [])].sort(
      (a: any, b: any) => {
        const ta = a?.start ? new Date(a.start).getTime() : 0;
        const tb = b?.start ? new Date(b.start).getTime() : 0;
        return ta - tb;
      ...
      const attendeeText = Array.isArray(e?.attendees)
        ? e.attendees
          .map((a: any) => {
            ...
            return `${a?.name || ""} ${a?.email || ""}`.toLowerCase();
          })
          .join(" ")
        : "";
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <aside className="space-y-6 lg:col-span-4 lg:sticky lg:top-0 lg:self-start lg:max-h-screen lg:overflow-y-auto pr-1">
        <EventTypesPanel userSub={userSub} />
