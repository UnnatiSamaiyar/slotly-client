...
export default function AvailabilityEditorModal({
  ...
  const [week, setWeek] = useState<Record<string, WeekRule>>(() =>
    fromAvailabilityJson(initialAvailabilityJson)
  );
  const [overrides, setOverrides] = useState<DateOverride[]>(() =>
    parseOverrides(initialAvailabilityJson)
  );
  const [blocks, setBlocks] = useState<DateBlock[]>(() =>
    parseBlocks(initialAvailabilityJson)
  );
  const [ranges, setRanges] = useState<DateRangeRule[]>(() =>
    parseRanges(initialAvailabilityJson)
  );
  const [bookingWindow, setBookingWindow] = useState<BookingWindow>(() =>
    parseBookingWindow(initialAvailabilityJson)
  );

  const [overrideDate, setOverrideDate] = useState("");
  const [overrideMode, setOverrideMode] =
    useState<DateOverride["mode"]>("available");
  const [overrideStart, setOverrideStart] = useState("09:00");
  const [overrideEnd, setOverrideEnd] = useState("17:00");
  ...
  function addOrUpdateOverride() {
    ...
    if (
      ...
      (!overrideStart || !overrideEnd || overrideEnd <= overrideStart)
    ) {
      return;
    }
    setOverrides((curr) => {
  ...
  function addBlock() {
    ...
    setBlocks((curr) => {
      const next = [
        ...curr,
        { date: blockDate, start: blockStart, end: blockEnd },
      ];
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/40 sm:p-4">
      <div className="flex h-full w-full items-end justify-center sm:items-center">
        <div className="w-full max-w-4xl bg-white rounded-t-2xl sm:rounded-2xl shadow-xl border border-gray-100 overflow-hidden h-[92dvh] sm:h-auto sm:max-h-[90dvh] flex flex-col">
          <div className="flex items-start sm:items-center justify-between gap-3 px-4 sm:px-5 py-4 border-b border-gray-100">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-5">
                ...
                {Object.keys(week)
                  ...
                  .map((k) => {
                    ...
                    return (
                      <div
                        key={k}
                        className="rounded-xl border border-gray-200 bg-white p-3 sm:p-0 sm:border-0 sm:bg-transparent"
                      >
                        <div className="sm:hidden">
                          ...
                          <div className="mt-3 flex items-center gap-2">
                            <input
                              ...
                              className="border border-gray-200 rounded-xl px-3 py-2 text-sm w-full disabled:bg-gray-50"
                            />
                            <span className="text-xs text-gray-400">to</span>
                            <input
                              type="time"
              ...
              <div className="mt-3 grid grid-cols-1 lg:grid-cols-12 gap-3 items-end">
                ...
                <div className="lg:col-span-2">
                  <button
                    ...
                    className="w-full px-4 py-2.5 rounded-xl text-sm font-semibold bg-slate-900 text-white hover:bg-slate-800"
                  >
                    Add/Update
                  </button>
                </div>
              ...
              <div className="mt-3 space-y-2">
                {overrides.length === 0 ? (
                  ...
                  overrides.map((o) => (
                    <div
                      key={o.date}
                      className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border border-gray-100 rounded-xl px-3 py-2"
                    >
                      <div className="text-sm min-w-0">
                        ...
                        <div className="text-xs text-slate-600">
                          {o.mode === "unavailable"
                            ? "Unavailable"
                            : `Available ${o.start}–${o.end}`}
                        </div>
                      </div>
                      <button
                        type="button"
                        onClick={() => removeOverride(o.date)}
                        className="self-start sm:self-auto text-xs font-semibold text-slate-600 hover:text-red-600"
                      >
                        Remove
                      </button>
                    </div>
            ...
            <div className="mt-5 bg-white border border-gray-100 rounded-2xl p-4">
              ...
              <div className="mt-1 text-xs text-slate-500">
                Example: 14:00–16:00 blocked on a specific date.
              </div>
            ...
            <div className="mt-5 bg-white border border-gray-100 rounded-2xl p-4">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                <div>
                  <div className="text-sm font-semibold text-slate-900">
                    Limit booking to a date window
                  </div>
                  <div className="mt-1 text-xs text-slate-500">
                    When enabled, only dates inside this range are selectable on
                    the public booking page. Other dates show as disabled/grey.
                  </div>
                </div>
                  ...
                  <button
                    ...
                    onClick={() =>
                      setBookingWindow((w) => ({
                        ...w,
                        start_date:
                          w.start_date || new Date().toISOString().slice(0, 10),
                        end_date:
                          w.end_date || new Date().toISOString().slice(0, 10),
                      }))
                    }
                    className="w-full px-4 py-2.5 rounded-xl text-sm font-semibold border border-gray-200 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Auto-fill
                  </button>
                </div>
              ...
              {bookingWindow.enabled &&
                bookingWindow.start_date &&
                bookingWindow.end_date &&
                bookingWindow.end_date < bookingWindow.start_date && (
            ...
            <div className="mt-5 bg-white border border-gray-100 rounded-2xl p-4">
              ...
              <div className="mt-1 text-xs text-slate-500">
                Applies when there is no explicit override for the date.
              </div>
          ...
          <div className="border-t border-gray-100 bg-white px-4 sm:px-5 py-4">
            <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-3">
              <button
                onClick={onClose}
                className="w-full sm:w-auto px-4 py-2.5 rounded-xl text-sm font-semibold border border-gray-200 hover:bg-gray-50"
              >
                Cancel
              </button>
              <button
                onClick={() => onSave(hasChangesPreview)}
                className="w-full sm:w-auto px-4 py-2.5 rounded-xl text-sm font-semibold bg-indigo-600 hover:bg-indigo-700 text-white"
              >
                Use this availability
              </button>
            </div>
