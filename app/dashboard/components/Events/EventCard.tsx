...
//       return;
//     }
//     const googleId = (event as any)?.googleEventId;
//     const bookingId = (event as any)?.id;

//     if (!googleId && !bookingId) {
//       toast({
//         title: "Action not available",
...
//           userSub,
//           googleEventId: String(googleId),
//           startISO: nextStartISO,
//           title: editForm.title,
//         });
//       } else {
//         await fetch(`${process.env.NEXT_PUBLIC_CALENDAR_API}/bookings/${bookingId}?user_sub=${userSub}`, {
//           method: "PUT",
//           headers: { "Content-Type": "application/json" },
//           body: JSON.stringify({
//             start_iso: nextStartISO,
//             title: editForm.title,
//           }),
//         });
//       }
...
//         userSub,
//         googleEventId: String((event as any).googleEventId),
//       });
...
// import { deleteBookingByGoogleEvent, updateBookingByGoogleEvent } from "../../api/calendar";
import { useToast } from "@/hooks/use-toast";
import { deleteBookingByGoogleEvent } from "../../api/calendar";
function safeDate(iso?: string) {
  if (!iso) return null;
  const d = new Date(iso);
  if (Number.isNaN(d.getTime())) return null;
...
export default function EventCard({ event, onClick, variant = "timeline", userSub, onChanged }: Props) {
  ...
  async function handleDelete(e: any) {
    ...
    if (!ok) {
      toast({
        title: "Cancelled",
        description: "Meeting was not deleted.",
        variant: "default",
      });
      return;
    }
