...
// import { ISODate } from "../../types";
// import { ChevronLeft, ChevronRight } from "lucide-react";
// import { toISODateLocal } from "./CalendarHelpers";
...
//   // ✅ instant selection (no delay)
//   const [localSelected, setLocalSelected] = useState<ISODate>(selectedDate);
...
//   const todayISO = useMemo(() => toISODateLocal(new Date()) as ISODate, []);

//   return (
//     <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
//       {/* HEADER */}
//       <div className="flex items-center justify-between mb-5">
//         <h2 className="text-lg font-semibold text-slate-900">{monthLabel(cursor)}</h2>
...
//       {/* WEEKDAYS (full width cells) */}
//       <div className="grid grid-cols-7 gap-2 mb-3">
//         {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((w) => (
//           <div
//             key={w}
//             className="h-10 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center
//                        text-sm font-semibold text-slate-800 tracking-wide"
//           >
//             {w}
//           </div>
//         ))}
...
//         {days.map((d, idx) => {
//           const iso = toISODateLocal(d) as ISODate;
//           const inMonth = d.getMonth() === month;
...
//               type="button"
//               onClick={() => {
//                 setLocalSelected(iso); // ✅ instant color change
//                 setSelectedDate(iso);
//               }}
//               className={[
//                 "relative h-14 rounded-xl border px-3",
//                 "flex flex-col items-center justify-center gap-1",
//                 "transition-all duration-150",
//                 isSelected
//                   ? "bg-indigo-600 text-white border-indigo-600 shadow-sm"
//                   : "bg-white hover:bg-slate-50 border-slate-200",
//                 !inMonth && !isSelected ? "text-slate-500 bg-slate-50 border-slate-200" : "",
//               ].join(" ")}
//             >
//               {/* DATE */}
//               <div className={["text-sm font-semibold", isToday && !isSelected ? "text-indigo-700" : ""].join(" ")}>
//                 {d.getDate()}
//               </div>
...
//               {hasEvents ? (
//                 <div className="flex items-center gap-2">
//                   <span className={["h-2.5 w-2.5 rounded-full", isSelected ? "bg-white/90" : "bg-indigo-600"].join(" ")} />
//                   <span
//                     className={[
...
//               {isToday && !isSelected ? (
//                 <span className="absolute inset-0 rounded-xl ring-2 ring-indigo-500/25" />
//               ) : null}
//             </button>
//           );
//         })}
...
//         <div className="flex items-center gap-2 shrink-0">
//           {/* Segmented nav */}
//           <div className="inline-flex items-center rounded-xl border border-white-200 bg-white shadow-sm overflow-hidden">
//             <button
//               onClick={() => {
//                 const d = new Date(cursor);
//                 d.setMonth(d.getMonth() - 1);
//                 setCursor(d);
//               }}
//               className="h-10 w-10 flex items-center justify-center hover:bg-white-50 transition"
//               aria-label="Previous month"
//               type="button"
//             >
//               <ChevronLeft className="w-4 h-4 text-slate-700" />
...
//             <div className="h-10 w-px bg-white-200" />
...
//                   // states
//                   inMonth ? "text-slate-900" : "text-white-400",
//                   inMonth ? "hover:bg-gray-50" : "hover:bg-gray-50/60",
...
export default function CalendarGrid({
  ...
  return (
    <div className="w-full rounded-2xl border border-slate-200 bg-white p-5 sm:p-6 shadow-[0_1px_0_rgba(15,23,42,0.04)]">      {/* Header */}
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            className="h-10 whitespace-nowrap rounded-xl border border-gray-200 bg-white px-3 text-sm font-medium transition hover:bg-gray-50"
            type="button"
          >
            Today
          </button>
      ...
      <div className="grid grid-cols-7 gap-1 sm:gap-2">
        {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
          <div
            key={`${d}-${i}`}
            className="py-1 text-center text-[11px] font-medium text-gray-500 sm:text-xs"
          >
            {d}
          </div>
        ))}
