...
// Backend active_weekdays format:
// 0 = Mon, 1 = Tue, 2 = Wed, 3 = Thu, 4 = Fri, 5 = Sat, 6 = Sun
function getScheduleWeekday(dateIso: string) {
  const jsDay = new Date(`${dateIso}T00:00:00`).getDay(); // Sun=0
  return (jsDay + 6) % 7; // Mon=0 ... Sun=6
}

function isScheduleDayUnavailable(dateIso: string, activeWeekdays?: number[] | null) {
  if (!Array.isArray(activeWeekdays) || activeWeekdays.length === 0) {
    return false;
  }

  return !activeWeekdays.includes(getScheduleWeekday(dateIso));
}
...
export default function PublicCalendar({
  ...
  return (
      ...
      <div className="grid grid-cols-7 gap-y-2">
        {days.map((d, i) => {
          ...
          const unavailableBySchedule =
            isCurrentMonth && isScheduleDayUnavailable(iso, activeWeekdays);

          const disabled =
            !isCurrentMonth || isPast || outOfWindow || unavailableBySchedule;
    ...
    transition-all duration-200
    ${disabled
                    ? "text-slate-400 bg-slate-50 cursor-not-allowed ring-1 ring-slate-100 hover:bg-slate-50"
                    : "text-gray-800 hover:bg-indigo-50 hover:scale-105"
                  }
    ${selected && !disabled
                    ? "bg-indigo-600 text-white shadow-md scale-105"
